...
def perform_test(model, train_loader, known_loader, unknown_loader, cfg):
    ...
    if os.path.exists("evm_models/timesformer_train_feats.bin"):
        ...
        extracted_feats = torch.zeros(n_train, 768+1)
        evm_feats, evm_labels = [], []
        j = 0
        for cur_iter, (inputs, labels, video_idx, meta) in enumerate(train_loader):
            bar.update(cur_iter)
            ...
            if cfg.NUM_GPUS > 1:
                preds, feats, labels, video_idx = du.all_gather([preds, feats, labels, video_idx])
            preds = preds.cpu()
            feats = feats.cpu()
            labels = labels.cpu()
            video_idx = video_idx.cpu()

            extracted_feats[j:(j+inputs.size(0)), 0] = labels
            extracted_feats[j:(j+inputs.size(0)), 1:] = feats
            j = j + inputs.size(0)
        ...
        for k in range(number_of_known_classes):
            ...
            evm_feats.append(cur_feat[:,1:].detach().clone())
    ...
    if os.path.exists("evm_models/timesformer_test_feats.bin"):
        ...
        for cur_iter, (inputs, labels, video_idx, meta) in enumerate(known_loader):
            bar.update(cur_iter)
            # Transfer the data to the current GPU device.
            if isinstance(inputs, (list,)):
                for i in range(len(inputs)):
        ...
        for cur_iter, (inputs, labels, video_idx, meta) in enumerate(unknown_loader):
            bar.update(cur_iter)
            # Transfer the data to the current GPU device.
            if isinstance(inputs, (list,)):
                for i in range(len(inputs)):
